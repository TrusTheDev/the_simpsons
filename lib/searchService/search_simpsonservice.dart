...
class Searchsimpsonservice {
  static List<Dtosearchmodel> searchMostSimilarMatches(
      ...
      if (similarity >= 0.6) {
        if (matches.length < responses) {
          ...
          for (int j = 1; j < matches.length; j++) {
            if (matches[j].similarity < matches[worstIndex].similarity) {
              worstIndex = j;
            }
          }

          if (similarity > matches[worstIndex].similarity) {
            matches[worstIndex] = Dtosearchmodel(
    ...
    matches.sort((b, a) => a.similarity.compareTo(b.similarity));
    return matches;
  }
}
